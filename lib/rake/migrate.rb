  ...
  module Compression
    class Migration
      ...
      def initialize
        ...
        require File.join(relative_root,'lib','ripple-compression.rb')
      ...
      # finde only the compressable models
      def models
        Objects.constants.map{|c| "#{c}".constantize}.select{|c| c.include?(Ripple::Compression)}
      end

      # cycle through all objects and save them
      def convert(type)
        # the difference between compression or decompression is
        # simply changing the content-type of the object so
        # that ripple knows what way to serialize it
        case type
        when :compress
          content_type = 'application/x-json-compressed'
        when :decompress
